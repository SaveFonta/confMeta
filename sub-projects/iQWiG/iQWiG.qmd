...
# Get the data
t <- read.csv("../data/IQWiG-data.csv")

# Store numerical data as matrix
dat <- as.matrix(
    t[c("treat.events", "treat.total", "control.events", "control.total")]
)
...
get_est <- function(x) {
    ...
    or <- (treat.events / treat.noevents) / (control.events / control.noevents)
    rr <- (treat.events / treat.total) / (control.events / control.total)

    logOR <- log(or)
    logRR <- log(rr)

    selogOR <- sqrt(sum(1 / c(treat.events, treat.noevents, control.events, control.noevents)))
    selogRR <- sqrt(1 / treat.events + 1 / control.events - 1 / treat.total - 1 / control.total)
...
get_ci <- function(measure, t, level) {
    ...
    # Which variable
    est_name <- paste0("log", measure)
    se_name <- paste0("selog", measure)
